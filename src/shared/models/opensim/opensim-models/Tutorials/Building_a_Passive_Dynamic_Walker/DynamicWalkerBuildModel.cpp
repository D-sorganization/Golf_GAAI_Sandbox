...
// Note: Several classes appear in both namespaces and require using the full
// name
...
int main() {
  try {
    ...
    double mass = 1;
    // Location of the Center of Mass from the Body Origin expressed in Body
    // Frame
    ...
    RightShank bodies double thighLength = 0.40, shankLength = 0.435;
            ...
            // Section B: Add Contact Geometry
                    ...
                    // The initial orientation is defined by a normal pointing
    in the positive x direction ContactHalfSpace* platformContact = new
    ContactHalfSpace(Vec3(0.0, 0.0, 0.0), Vec3(0.0, 0.0, -Pi / 2.0), *platform,
    "PlatformContact"); osimModel.addContactGeometry(platformContact);
                    ...
                    ContactSphere* rightHipContact = new
    ...
    *rightHipContactParams = new
    OpenSim::HuntCrossleyForce::ContactParameters(stiffness, dissipation,
    staticFriction, dynamicFriction, viscosity);
                    rightHipContactParams->addGeometry("RHipContact");
                    rightHipContactParams->addGeometry("PlatformContact");
                    // Right Hip Force
                    OpenSim::HuntCrossleyForce* rightHipForce = new
    OpenSim::HuntCrossleyForce(rightHipContactParams);
