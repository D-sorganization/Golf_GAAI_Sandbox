 ...
 */
void setActiveForceLengthCurveParametersAllMuscles(
    Model *osimModel, double minNormActiveFiberLength,
    double transNormFiberLength, double maxNormActiveFiberLength,
    double shallowAscendingSlope, double minNormForce) {
  ...
  for (int i = 0; i < numMuscles; i++) {
    ...
    Millard2012EquilibriumMuscle *currMuscle =
        static_cast<Millard2012EquilibriumMuscle *>(&allMuscles.get(i));

    ActiveForceLengthCurve currCurve = currMuscle->getActiveForceLengthCurve();

    cout << "\nCurrent muscle: " << currMuscle->getName() << endl;

    cout << "\t current min norm active fiber length: "
         << currCurve.get_min_norm_active_fiber_length() << endl;
    currCurve.set_min_norm_active_fiber_length(minNormActiveFiberLength);

    cout << "\t current transition norm fiber length: "
         << currCurve.get_transition_norm_fiber_length() << endl;
    currCurve.set_transition_norm_fiber_length(transNormFiberLength);

    cout << "\t current max norm active fiber length:  "
         << currCurve.get_max_norm_active_fiber_length() << endl;
    currCurve.set_max_norm_active_fiber_length(maxNormActiveFiberLength);

    cout << "\t current shallow ascending slope: "
         << currCurve.get_shallow_ascending_slope() << endl;
    currCurve.set_shallow_ascending_slope(shallowAscendingSlope);

    cout << "\t current min norm force: " << currCurve.get_minimum_value()
         << endl;
 ...
 *   muscle
 * ASSUMES THAT FOR THE N MUSCLES IN THE MODEL, MUSCLES INDEXED 0 to N/2-1 ARE
 * THE RIGHT MUSCLES AND MUSCLES INDEXED N/2 to N-1 ARE THE LEFT MUSCLES IN THE
 * SAME ORDER Input:  originalModel (.osim model) Output: newModel (.osim model)
 */
void setPropertiesOfLeftMusclesBasedOnRightMuscles(Model *osimModel) {
  ...
  for (int i = 0; i < numMuscles / 2; i++) {
    ...
    leftMuscle.setPennationAngleAtOptimalFiberLength(
        rightMuscle.getPennationAngleAtOptimalFiberLength());

    cout << "\n";
    cout << rightMuscle.getName() << "\t" << rightMuscle.getMaxIsometricForce()
         << "\t" << rightMuscle.getTendonSlackLength()
         << rightMuscle.getOptimalFiberLength() << "\t"
         << rightMuscle.getPennationAngleAtOptimalFiberLength() << endl;
    cout << leftMuscle.getName() << "\t" << leftMuscle.getMaxIsometricForce()
         << "\t" << leftMuscle.getTendonSlackLength()
         << leftMuscle.getOptimalFiberLength() << "\t"
         << leftMuscle.getPennationAngleAtOptimalFiberLength() << endl;
  }
}
 ...
 * Output: newModel (.osim model)
 */
void setRigidTendonBasedOnLtsToLmoRatio(Model *osimModel,
                                        double ratioForRigidTendon) {
  Set<Muscle> &allMuscles = osimModel->updMuscles();
  int numMuscles = allMuscles.getSize();
  cout << "\nNumber of Muscles:  " << numMuscles << endl;
...
void setFiberDampingAllMuscles(Model *osimModel, double fiberDamping) {
  ...
  for (int i = 0; i < numMuscles; i++) {
    Millard2012EquilibriumMuscle &currentMuscle =
        dynamic_cast<Millard2012EquilibriumMuscle &>(Muscles.get(i));
 ...
 *
 */
void modifyPassiveFLCurveAllMuscles(Model *osimModel, double strainAtZeroForce,
  ...
  for (int i = 0; i < numMuscles; i++) {
    Millard2012EquilibriumMuscle &currentMuscle =
        dynamic_cast<Millard2012EquilibriumMuscle &>(Muscles.get(i));
    currentMuscle.upd_FiberForceLengthCurve().set_strain_at_zero_force(
        strainAtZeroForce);
    currentMuscle.upd_FiberForceLengthCurve().set_strain_at_one_norm_force(
 ...
 *
 */
void setMaxContractionVelocityAllMuscles(Model *osimModel,
 ...
 * Standard, unscaled model should have mass of 75.337 kg and height of 170 cm
 * (approx)
 */
double getMassOfModel(Model *osimModel) {
  ...
  cout << "sum masses of all bodies in model, total mass = " << totalMass
       << endl;
 ...
 * Useful if you want to change the specific tension used to scale all muscles,
 * for example
 */
void scaleOptimalForceByConstant(Model *osimModel, double forceScaleFactor) {
  ...
  for (int i = 0; i < allMuscles.getSize(); i++) {
    Muscle &currentMuscle = allMuscles.get(i);
    currentMuscle.setMaxIsometricForce(forceScaleFactor *
                                       currentMuscle.getMaxIsometricForce());
  }
}
 ...
 */
void scaleOptimalForceSubjectSpecific(Model *originalModel, Model *newModel,
                                      double heightOriginal, double heightNew) {
  ...
  for (int i = 0; i < allMusclesOriginal.getSize(); i++) {
    ...
    currentMuscleNew.setMaxIsometricForce(
        forceScaleFactor * currentMuscleOriginal.getMaxIsometricForce());

    cout << endl << currentMuscleNew.getName() << endl;
    cout << "max isometric force: " << currentMuscleNew.getMaxIsometricForce()
         << endl;
 ...
 */
void printMusclePropertiesToFile(
    Model *osimModel, double specificTensionUsedInNewtonPerCmSquared) {

  ofstream musclePropertiesFile("musclePropertiesFile.txt", ofstream::out);

  musclePropertiesFile << "Muscle" << "\t" << "PCSA (cm^2)" << "\t"
                       << "Muscle Volume (cm^3)" << "\t" << "Peak force (N)"
                       << "\t" << "Optimal fiber length (cm)" << "\t"
                       << "Tendon slack length (cm)" << "\t"
                       << "Pennation angle (degrees)" << "\t"
                       << "Rigid tendon approximation used" << "\t"
                       << "Fiber damping used" << endl;
  musclePropertiesFile << "----------------------------------------------------"
                          "-------------------------------------------"
                       << endl;
  ...
  double peakForceInNewtons, optFiberLengthInCm, tendonSlackLengthInCm,
      pennationAngleInDeg, PCSAInCm2, volumeInCm3, fiberDamping;
  bool rigidTendon;

  for (int i = 0; i < allMuscles.getSize(); i++) {
    Millard2012EquilibriumMuscle &currentMuscle =
        dynamic_cast<Millard2012EquilibriumMuscle &>(allMuscles.get(i));
    ...
    pennationAngleInDeg =
        currentMuscle.getPennationAngleAtOptimalFiberLength() * radTOdeg;
    rigidTendon = currentMuscle.get_ignore_tendon_compliance();
    if (rigidTendon) {
      rigidTendonYesNo = "yes";
    } else {
      rigidTendonYesNo = "no";
    }
    ...
    musclePropertiesFile << muscleName << "\t" << PCSAInCm2 << "\t"
                         << volumeInCm3 << "\t" << peakForceInNewtons << "\t"
                         << optFiberLengthInCm << "\t" << tendonSlackLengthInCm
                         << "\t" << pennationAngleInDeg << "\t"
                         << rigidTendonYesNo << "\t" << fiberDamping << endl;
  }
