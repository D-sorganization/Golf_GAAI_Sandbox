...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("Muscle Equilibrium Solver Test")
    logger.info("=" * 60)
    logger.info("Muscle parameters:")
    logger.info(f"  F_max = {params.F_max:.0f} N")
    logger.info(f"  l_opt = {params.l_opt * 100:.1f} cm")
    logger.info(f"  l_slack = {params.l_slack * 100:.1f} cm")
    logger.info("\\nTest case:")
    logger.info(f"  l_MT = {l_MT_test * 100:.2f} cm")
    logger.info(f"  activation = {activation_test * 100:.0f}%")

    l_CE_solution = solver.solve_fiber_length(l_MT_test, activation_test)

    logger.info("\\nSolution:")
    logger.info(f"  l_CE = {l_CE_solution * 100:.2f} cm")
    logger.info(f"  l_CE / l_opt = {l_CE_solution / params.l_opt:.3f}")

    # Verify equilibrium
    residual = solver._equilibrium_residual(l_CE_solution, l_MT_test, activation_test)
    logger.info(f"  Residual = {residual:.2e} N (should be ~0)")

    logger.info("\\n" + "=" * 60)
    logger.info("✓ Equilibrium solver test complete")
    logger.info("=" * 60)
