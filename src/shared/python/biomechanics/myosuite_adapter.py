...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("MyoSuite Integration Example")
    logger.info("=" * 60)
    ...
    logger.info("\\nEnvironment created:")
    logger.info(f"  Task: {env.task}")
    logger.info(f"  Timestep: {env.dt * 1000:.1f} ms")
    logger.info(f"  Muscles: {len(env._get_muscle_names())}")
    ...
    logger.info(f"    Torque: {info['tau_muscle']:.2f} N·m")

    logger.info("\\n" + "=" * 60)
    logger.info("✓ MyoSuite integration test complete")
    logger.info("=" * 60)
    logger.info("\\nTo train a policy:")
    logger.info("  policy = train_muscle_policy(env, total_timesteps=50000)")
    logger.info("  # Then use: action, _states = policy.predict(observation)")
