...
Extracted from ``mesh_generator.py`` as part of issue #136, following
the pattern established in PR #172 (``PrimitiveMeshGenerator`` extraction).

Note: module-level availability flags (``SMPLX_AVAILABLE``, ``TRIMESH_AVAILABLE``)
and mock-patchable module handles (``_smplx_module``, ``_trimesh_module``) still
live on ``mesh_generator`` so existing ``patch("...mesh_generator._smplx_module")``
tests keep working; we dereference them via the module object at call time.
"""
