"""Configuration dataclasses for the aerodynamics module.

Defines the immutable configuration objects consumed by the aerodynamics
force models, wind model, and environment randomizer:

- :class:`AerodynamicsConfig` — master / per-effect toggles + tunable coefficients
- :class:`WindConfig` — constant wind, gusts, turbulence, altitude gradient
- :class:`RandomizationConfig` — stochastic environment knobs

All configs are frozen dataclasses so that simulations can share them
safely across threads and easily snapshot/replay a run.
"""
