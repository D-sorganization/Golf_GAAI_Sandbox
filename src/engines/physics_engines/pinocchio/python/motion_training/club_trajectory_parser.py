...
class ClubTrajectoryParser:
    ...
    @staticmethod
    def _make_row_accessor(row) -> Any:
        if hasattr(row, "iloc"):
    ...
    @staticmethod
    def _orthogonalize_axes(x_axis, y_axis) -> Any:
    ...
    def _parse_grip_data(self, get) -> Any:
    ...
    def _parse_club_face_data(self, get, grip_pos, grip_rot) -> Any:
        if not (get is not None):
            raise ValueError("get must be provided")
        if not (get is not None):
            raise ValueError("get must be provided")
