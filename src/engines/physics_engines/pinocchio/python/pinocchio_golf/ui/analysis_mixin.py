"""Analysis and plotting mixin for Pinocchio GUI."""
...
class AnalysisMixin:
    """Mixin containing post-hoc analysis and plotting logic."""

    def _setup_analysis_tab(self: PinocchioGUI) -> None:
    ...
    def _ensure_analyzer_initialized(self: PinocchioGUI) -> None:
    ...
    def _plot_induced_accelerations(self: PinocchioGUI) -> None:
        ...
        txt = getattr(self, "combo_induced", None) and self.combo_induced.currentText()
        if (
            not has_specific
            and txt
    ...
    def _plot_counterfactuals(self: PinocchioGUI) -> None:
    ...
    def _export_statistics(self: PinocchioGUI) -> None:
