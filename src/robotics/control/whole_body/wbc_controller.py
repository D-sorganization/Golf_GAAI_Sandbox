...
class WholeBodyController:
    ...
    def _build_priority_level_cost(self, tasks, n_v, n_vars, accumulated_A) -> Any:
    ...
    def _apply_regularization(self, H, n_v, n_contact_vars) -> Any:
        H[:n_v, :n_v] += self._config.regularization * np.eye(n_v)
        if n_contact_vars > 0:
            H[n_v:, n_v:] += self._config.contact_force_regularization * np.eye(n_contact_vars)

    def _build_level_qp(self, H, g, n_v, n_contact_vars, M, nle, qd) -> Any:
