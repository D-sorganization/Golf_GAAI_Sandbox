...
class TestPhase1SecurityIntegration(unittest.TestCase):
    ...
    def test_golf_launcher_security_integration(
        ...
        # Import GolfLauncher locally to ensure we patch the exact class object

        # Mock the launcher initialization AND setWindowIcon to avoid GUI setup and TypeErrors
        with (
            patch.object(GolfLauncher, "__init__", return_value=None),
            patch.object(GolfLauncher, "setWindowIcon"),
        ):
