...
class TestMuJoCoSimWidget:
    ...
    def test_reset_state_returns_to_initial(self, qapp):
        ...
        assert widget.data is not None, "Model data should be loaded after load_model_from_xml"
    ...
    def test_camera_setting(self, qapp):
        ...
        assert successful_views, (
            "At least one camera view should be set successfully; " f"failed views: {failed_views}"
        )
...
class TestHumanoidLauncher:
    ...
    def test_launcher_has_required_components(self, qapp):
        ...
        assert not missing_attrs, f"Launcher is missing expected widget attributes: {missing_attrs}"
